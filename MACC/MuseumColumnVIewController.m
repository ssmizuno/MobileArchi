...
#import "RikenYamamotoViewController.h"
//----------　----------//
//変数用意
//----------------------------------------------------------------------//
static NSString *Surl = @"http://www.ss-maple.co.jp/ma_resources/sound/museumcolumn.mp3";
//----------------------------------------------------------------------//

//クラスの実装
@implementation MuseumColumnVIewController
...
- (void)viewDidLoad {
    ...
    NSString *textPath = [[NSBundle mainBundle] pathForResource:@"museumColumn" ofType:@"txt"];
    ...
    for (int i = 0; i < 3; i++) {
        ...
        UIImage *scrollImage;
        if(i < 2){
            scrollImage = [UIImage imageNamed:[NSString stringWithFormat:@"musec0%02d.png", i+1]];
        }else {
            scrollImage = [UIImage imageNamed:[NSString stringWithFormat:@"musec0%02d.png", i+1]];
...
-(void)architectShow:(id)sender{	
	RikenYamamotoViewController *next01 = [[[RikenYamamotoViewController alloc] initWithNibName:nil bundle:nil] autorelease];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    switch (buttonIndex) {
		case 0: {
            ...
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://www.google.co.jp/m/search?site=images&oe=UTF-8&client=safari&hl=ja&q=%E6%A8%AA%E9%A0%88%E8%B3%80%E7%BE%8E%E8%A1%93%E9%A4%A8"]];
			break;
		}
