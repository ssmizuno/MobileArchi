...
@synthesize cTitle, cCont, imageSearch, wikiSearch, sUrl;
...
- (void)initDataChairs:(NSString *)_cTitle name:(NSString *)_name imageSearch:(NSString *)_imageSearch wikiSearch:(NSString *)_wikiSearch sUrl:(NSString *)_sUrl isCameraUse:(BOOL)_isCameraUse {
    self.cTitle = _cTitle;
    self.cCont = _name;
    self.imageSearch = _imageSearch;
    self.wikiSearch = _wikiSearch;
    self.sUrl = _sUrl;
    isCameraUse = _isCameraUse;
}
...
- (void)viewDidLoad {
    ...
    columnTitle.text = cTitle;
    columnTitle.numberOfLines = 0;
    columnTitle.textAlignment = UITextAlignmentCenter;
    columnTitle.font = [UIFont fontWithName:@"Helvetica-Bold" size:22.0];
    ...
    //説明文
    //----------　----------//
    UITextView *textview = [[UITextView alloc] initWithFrame:CGRectMake(10, 225, 300, 160)];
    NSString *textPath = [[NSBundle mainBundle] pathForResource:cCont ofType:@"txt"];
    ...
    //----------　----------//
    scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 44, 320, 190)];
    ...
    for (int i = 0; i < 3; i++) {
        ...
        UIImage *scrollImage;
        if(i < 2){
            scrollImage = [UIImage imageNamed:[NSString stringWithFormat:@"%@%02d.png", cCont, i+1]];
        }else {
            scrollImage = [UIImage imageNamed:[NSString stringWithFormat:@"%@%02d.png", cCont, i+1]];
            
        }
        
        UIImageView *scrollImageView = [[UIImageView alloc] initWithFrame:CGRectMake(i*320, 0, 320, 190)];
    ...
    //ページコントロールの追加
    pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(60, 204, 200, 50)];
	...
	UIBarButtonItem *camerabutton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCamera
                                                                                target:self
                                                                                action:@selector(camerastart:)];
    //-- ツールバースタイルの適用 --//
	camerabutton.style = itemStyle;
    ...
    NSArray *barItems;
    if (isCameraUse) {
	barItems = [NSArray arrayWithObjects: playbutton, stopbutton, flexibleSpace, camerabutton, defaultItem,  nil];
    } else {
    barItems = [NSArray arrayWithObjects: playbutton, stopbutton, flexibleSpace, defaultItem,  nil];
    }
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    switch (buttonIndex) {
		case 0: {
            ...
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:imageSearch]];
			break;
		}
		case 1: {
            //建築家情報を表示（Wikipedia表示）
            NSLog(@"osita");
			[[UIApplication sharedApplication] openURL:[NSURL URLWithString:wikiSearch]];
			break;
		}
	}
}
...
//カメラ起動action
//----------　----------//
- (BOOL)camerastart:(id)sender {
    //カメラが使えるかチェック
    if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"You do not have camera" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
    ...
    cameraViewController.imgname = cCont;
    [self presentModalViewController:cameraViewController animated:NO];
