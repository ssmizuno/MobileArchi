...
//変数用意
//----------------------------------------------------------------------//
static NSString *Surl = @"http://www.ss-maple.co.jp/ma_resources/sound/topcolumn.mp3";
//----------------------------------------------------------------------//

//クラスの実装
@implementation TopColumnViewController
...
- (void)viewDidLoad {
    ...
    columnTitle.text = @"ビヤルケ・インゲルスに第１回ウォール・ストリートジャーナル・イノヴェータ賞";
    ...
    NSString *textPath = [[NSBundle mainBundle] pathForResource:@"topColumn" ofType:@"txt"];
    ...
    for (int i = 0; i < 2; i++) {
        ...
        UIImage *scrollImage;
        if(i < 1){
            scrollImage = [UIImage imageNamed:[NSString stringWithFormat:@"topc0%02d.png", i+1]];
        }else {
            scrollImage = [UIImage imageNamed:[NSString stringWithFormat:@"topc0%02d.png", i+1]];
	...
	// アイテムは配列に入れてバーに設定します
	//---------------------------------------------//
	NSArray *barItems = [NSArray arrayWithObjects: /*playbutton, stopbutton, */flexibleSpace, defaultItem,  nil];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    switch (buttonIndex) {
		case 0: {
            ...
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://www.google.co.jp/m/search?gcx=c&q=bjarke+ingels&um=1&ie=UTF-8&hl=ja&tbm=isch&source=og&sa=N&tab=wi&biw=1238&bih=639&sei=ll3PTpHkGKiemQWpjeXACg"]];
			break;
		}
		case 1: {
            //建築家情報を表示（Wikipedia表示）
			[[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://en.wikipedia.org/wiki/Bjarke_Ingels"]];
			break;
		}
	}
}
